...
class OldBloomFilterImpl extends BloomFilter implements Serializable {

  private int numHashFunctions;

  private BitArray bits;

  OldBloomFilterImpl(int numHashFunctions, long numBits) {
    this(new BitArray(numBits), numHashFunctions);
  }

  private OldBloomFilterImpl(BitArray bits, int numHashFunctions) {
    this.bits = bits;
    this.numHashFunctions = numHashFunctions;
  }

  private OldBloomFilterImpl() {}
  ...
  public boolean equals(Object other) {
    ...
    if (other == null || !(other instanceof OldBloomFilterImpl)) {
      return false;
    }

    OldBloomFilterImpl that = (OldBloomFilterImpl) other;
  ...
  public static OldBloomFilterImpl readFrom(InputStream in) throws IOException {
    OldBloomFilterImpl filter = new OldBloomFilterImpl();
    filter.readFrom0(in);
    return filter;
  }
